...
class ObjectTransitionV5Env(gym.Env):
    metadata = {
    ...
    def __init__(self):
        ...
        self.max_vel = 4.0 # max velocity of the object

        # define environment area and obstacles
        self.region = [0, 30, 0, 20] # l,r,b,u
        self.obstacles = []
        self.obstacles.append([14, 16, 6, 13])
        self.goal = [20, 25, 7, 12]
        self.goal_center = [(self.goal[0]+self.goal[1])/2.0, (self.goal[2]+self.goal[3])/2.0]
        self.goal_radius = math.sqrt( (self.goal[1]-self.goal[0])**2 + (self.goal[3]-self.goal[2])**2 )
    ...
    def _step(self, raw_action):
        ...
        done = (is_in_goal and _norm(velocity) < 0.1) or is_in_obstacles
    ...
    def _reset(self):
        # randomly assign intial position
        rand_pos = self.goal_center
        tt = 1
        while tt > 0: # make sure intial position in neither in goal or obstacle
            rand_pos = [self.np_random.uniform(low=self.region[0], high=self.region[1]), self.np_random.uniform(low=self.region[2], high=self.region[3])]
            tt = 0
            tt += _isIn(rand_pos, self.goal)
            for obstacle in self.obstacles:
                tt += _isIn(rand_pos, obstacle)

        # randomly assign intial velocity
        rand_vel = [0.0, 0.0]
        rand = np.random.rand()
        if rand < 0.2:
            rand_vel = [self.np_random.uniform(low=0.0, high=self.max_vel/0.7), self.np_random.uniform(low=0.0, high=self.max_vel/0.7)]

        self.state = np.array([rand_pos[0], rand_vel[0], rand_pos[1], rand_vel[1]])
...
# class ObjectTransitionV2Env(gym.Env):
#     metadata = {
#         'render.modes': ['human', 'rgb_array'],
#         'video.frames_per_second': 30
#     }
